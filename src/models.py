...
class AppConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "AppConfig":
        ...
        threshold_value = vad.get("threshold")
        if not isinstance(threshold_value, (int, float)) or isinstance(
            threshold_value, bool
        ):
            raise ConfigurationError("vad.threshold must be a number.")
        ...
        config = cls(
            ...
            stream=StreamConfig(
                ...
                restart_delay_seconds=_require(
                    stream, "restart_delay_seconds", int
                ),
            ),
            vad=VadConfig(
                ...
                silence_timeout_ms=_require(vad, "silence_timeout_ms", int),
                pre_buffer_ms=_require(vad, "pre_buffer_ms", int),
                post_buffer_ms=_require(vad, "post_buffer_ms", int),
            ),
            recording=RecordingConfig(
                ...
                minimum_duration_ms=_require(
                    recording, "minimum_duration_ms", int
                ),
                maximum_duration_seconds=_require(
                    recording, "maximum_duration_seconds", int
                ),
                metadata_enabled=_require(
                    recording, "metadata_enabled", bool
                ),
            ),
            upload=UploadConfig(
                ...
                retry_initial_seconds=_require(
                    upload, "retry_initial_seconds", int
                ),
                retry_max_seconds=_require(upload, "retry_max_seconds", int),
                delete_after_success=_require(
                    upload, "delete_after_success", bool
                ),
            ),
            logging=LoggingConfig(
    ...
    def validate(self) -> None:
        ...
        if self.audio.chunk_duration_ms <= 0:
            raise ConfigurationError(
                "audio.chunk_duration_ms must be positive."
            )

        if self.audio.frames_per_chunk <= 0:
            raise ConfigurationError(
                "Audio settings result in zero frames per chunk."
            )
        ...
        if self.vad.pre_buffer_ms < 0 or self.vad.post_buffer_ms < 0:
            raise ConfigurationError(
                "VAD pre-buffer and post-buffer values cannot be negative."
            )
        ...
        if (
            self.upload.retry_max_seconds
            < self.upload.retry_initial_seconds
        ):
            raise ConfigurationError(
        ...
        if self.logging.backup_count < 1:
            raise ConfigurationError(
                "logging.backup_count must be at least 1."
            )
